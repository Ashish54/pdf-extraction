...
```yaml
title: "My Document"          # documentation; stamps output /Title metadata
...
```bash
cd pdf_extractor
../.venv/bin/python -m pdf_extractor.cli ../myconfig.yaml /path/to/input.pdf --dry-run
```
...
```bash
../.venv/bin/python -m pdf_extractor.cli ../myconfig.yaml /path/to/input.pdf -o /path/to/output.pdf
```

- Output defaults to `<input>.extracted.pdf` next to the input if `-o` is omitted.
...
../.venv/bin/python -m pdf_extractor.cli sample/config.yaml sample/sample.pdf --dry-run
../.venv/bin/python -m pdf_extractor.cli sample/config.yaml sample/sample.pdf -o sample/extracted.pdf
```
...
report = extract(config, "input.pdf", "output.pdf", dry_run=True)
