"""Command-line interface: pdf-extract CONFIG.yaml FILENAME [-o OUT.pdf]

FILENAME is resolved inside the config's input_dir; the output defaults to
<output_dir>/<name>_reduced.pdf. The output /Title defaults to the file name.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "input",
        help="PDF file name inside the configured input_dir (or a path to a PDF)",
    )
    p.add_argument(
        "-o",
        "--output",
        help="output PDF (default: <output_dir>/<name>_reduced.pdf)",
    )
    p.add_argument(
        "--dry-run",
        action="store_true",
