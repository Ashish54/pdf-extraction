...
```bash
pdf-extract config.yaml annual_report.pdf
# or without installing:
python -m pdf_extractor.cli config.yaml annual_report.pdf
```

You pass **just the file name** — it is looked up inside the config's
`input_dir`, and the result is written to
**`<output_dir>/<name>_reduced.pdf`** with its `/Title` metadata taken from
the file name. Passing a full path also works, and `-o` overrides the output
location.
...
```bash
pdf-extract config.yaml annual_report.pdf --dry-run
```

## Config (YAML)

```yaml
input_dir: "./pdfs/in"      # bare file names are resolved here
output_dir: "./pdfs/out"    # <output_dir>/<name>_reduced.pdf
# title: "Custom Title"     # optional; default is the input file name
...
report = extract(config, "annual_report.pdf")  # resolved inside input_dir
print(report.output_path, report.kept_pages, report.selection.boundaries)
```
