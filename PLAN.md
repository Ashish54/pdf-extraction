...
## 13. v1.1 amendments (post-release requests)

- **Title from file name**: output `/Title` defaults to the input file's stem; config `title:` is now an optional override, no longer the primary source.
- **Config-driven directories**: new `input_dir` / `output_dir` keys (default `.`). CLI takes a bare file name, resolved inside `input_dir`.
- **Output naming**: default output is `<output_dir>/<stem>_reduced.pdf` (supersedes `<stem>.extracted.pdf`); `-o` still overrides.
